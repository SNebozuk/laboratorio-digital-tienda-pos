...
<body><main class="quote-shell"><a class="quote-back" href="<?= $escape($storePath ?: '/') ?>">← Volver a Laboratorio Digital</a><header class="quote-hero"><span>✦ HERRAMIENTA ATELIER</span><h1>Tu idea, <em>bien cotizada.</em></h1><p>Calculá el papel y una ganancia saludable para cada trabajo de tarjetería o papelería.</p></header>
<section class="quote-card"><div class="quote-step"><b>1</b><div><h2>Elegí tu papel</h2><p>Elegí entre los papeles disponibles en Laboratorio Digital.</p></div></div><details id="paper-picker" class="paper-picker"><summary>Elegí el papel que más te guste <span>⌄</span></summary><div id="paper-list" class="paper-list"><table class="paper-table"><thead><tr><th>Papel</th><th>Precio resma</th></tr></thead><tbody><?php foreach ($quotePapers as $paper): ?><tr data-id="<?= (int) $paper['variant_id'] ?>" data-product="<?= $escape((string) $paper['product_name']) ?>" data-variant="<?= $escape((string) $paper['variant_name']) ?>" data-price="<?= (int) $paper['price_cents'] ?>"><td><?= $escape((string) $paper['product_name']) ?><?= $paper['variant_name'] ? ' · ' . $escape((string) $paper['variant_name']) : '' ?><button class="choose-paper" type="button">Elegir</button></td><td>$ <?= number_format((int) $paper['price_cents'] / 100, 0, ',', '.') ?></td></tr><?php endforeach ?></tbody></table></div></details><p id="paper-selected" class="paper-selected" hidden></p></section>
