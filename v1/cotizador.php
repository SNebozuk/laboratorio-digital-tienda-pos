...
$quoteAssetsVersion = (string) max(
    ...
    filemtime(__DIR__ . '/assets/quote.css') ?: 0,
    filemtime(__DIR__ . '/assets/quote-table.css') ?: 0,
...
foreach ($quoteCatalog['products'] as $product) {
    ...
    foreach ($product['variants'] as $variant) {
        ...
        $quotePapers[] = ['product_name' => $product['name'], 'variant_name' => $variant['name'], 'variant_id' => $variant['id'], 'price_cents' => $variant['price_cents'], 'image_path' => $product['image_path']];
    }
}
...
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Cotizador para Emprendedores · Laboratorio Digital</title><link rel="icon" href="<?= $escape($assetPath) ?>/favicon.png?v=20260826" type="image/png"><link rel="stylesheet" href="<?= $escape($assetPath) ?>/quote.css?v=<?= $quoteAssetsVersion ?>"><link rel="stylesheet" href="<?= $escape($assetPath) ?>/quote-table.css?v=<?= $quoteAssetsVersion ?>"><link rel="stylesheet" href="<?= $escape($assetPath) ?>/quote-carousel.css?v=<?= $quoteAssetsVersion ?>"></head>
<body><main class="quote-shell"><a id="quote-back" class="quote-back" href="<?= $escape($storeUrl) ?>?volver-del-cotizador=1">← Volver a Laboratorio Digital</a><header class="quote-hero"><span>✦ HERRAMIENTA ATELIER</span><h1>Tu idea, <em>bien cotizada.</em></h1><p>Calculá cuánto papel necesita y cuánto cuesta tu trabajo de tarjetería o papelería.</p></header>
<section class="quote-card"><div class="quote-step"><b>1</b><div><h2>Elegí tu papel</h2><p>Elegí entre los papeles disponibles en Laboratorio Digital.</p></div></div><details id="paper-picker" class="paper-picker"><summary>Seleccioná un papel <span>⌄</span></summary><div id="paper-list" class="paper-list"><table class="paper-table"><thead><tr><th>Vista</th><th>Papel</th><th>Precio resma</th></tr></thead><tbody><?php foreach ($quotePapers as $paper): ?><tr data-id="<?= (int) $paper['variant_id'] ?>" data-product="<?= $escape((string) $paper['product_name']) ?>" data-variant="<?= $escape((string) $paper['variant_name']) ?>" data-price="<?= (int) $paper['price_cents'] ?>" data-image="<?= $escape((string) ($paper['image_path'] ?? '')) ?>"><td class="paper-thumb-cell"><?php if ($paper['image_path']): ?><span class="paper-thumbnail"><img src="<?= $escape((string) $paper['image_path']) ?>" alt=""><span class="paper-thumbnail-zoom"><img src="<?= $escape((string) $paper['image_path']) ?>" alt="<?= $escape((string) $paper['product_name']) ?>"></span></span><?php endif ?></td><td><?= $escape((string) $paper['product_name']) ?></td><td>$ <?= number_format((int) $paper['price_cents'] / 100, 0, ',', '.') ?></td></tr><?php endforeach ?></tbody></table></div></details><div id="paper-selected" class="paper-selected" hidden><img id="paper-selected-image" class="paper-selected-image" alt=""><div><span>Papel elegido</span><strong id="paper-selected-name"></strong><small id="paper-selected-price"></small></div></div></section>
...
<section class="quote-card quote-project"><div class="quote-step"><b>2</b><div><h2>Contanos tu proyecto</h2><p>Usamos el tamaño real del papel elegido para aprovecharlo al máximo.</p></div></div><div class="quote-fields"><label>Cantidad de piezas<input id="project-quantity" type="number" min="1" value="200"></label><label>Ancho de cada pieza (cm)<input id="project-width" type="number" min="0.1" step="0.1" value="10"></label><label>Alto de cada pieza (cm)<input id="project-height" type="number" min="0.1" step="0.1" value="15"></label><label>Margen de corte (mm)<input id="cut-margin" type="number" min="0" step="0.5" value="0"></label></div></section>
<section id="cut-preview" class="quote-card cut-preview" aria-live="polite"><div class="quote-step"><b>3</b><div><h2>Vista previa de cortes</h2><p>Seleccioná un papel para ver el esquema a escala.</p></div></div></section>
<button id="calculate-quote" class="quote-calculate" type="button">Calcular costo del papel</button>
<dialog id="quote-modal" class="quote-modal" aria-labelledby="quote-modal-title"><div class="klaus-checkout" aria-hidden="true"><img class="klaus-image" src="<?= $escape($assetPath) ?>/klaus_checkout_sitting.png" alt=""></div><section id="quote-result" class="quote-result" aria-live="polite"><h2 id="quote-modal-title">Tu cotización</h2></section><button id="close-quote" class="quote-calculate" type="button">Volver al cotizador</button></dialog>
