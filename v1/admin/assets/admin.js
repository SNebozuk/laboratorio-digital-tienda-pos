(() => {
    ...
    function closeModal() {
        ...
        if (state.aiDialogOpen) {
            ...
            if (aiView && state.aiDialogPlaceholder) {
                state.aiDialogPlaceholder.replaceWith(aiView);
                aiView.classList.remove('ai-search-dialog');
            }
            state.aiDialogOpen = false;
            state.aiDialogPlaceholder = null;
    ...
    function renderAiSearch() {
        ...
        const variantTokens = aiSearchVariantTokens(query);
        if (!query) {
            elements.aiSearchMessages.innerHTML = '<div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p>¡Hola! Soy el Asesor IA de Laboratorio Digital. ¿En qué te ayudo?</p></div>';
        ...
        if (!state.productsLoaded) {
            elements.aiSearchMessages.innerHTML = `<div class="ai-search-message ai-search-message-client"><small>CLIENTE</small><p>${escapeHtml(query)}</p></div><div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p class="ai-search-progress"><i aria-hidden="true"></i>Ya te digo, voy a buscarlo en el catálogo.</p></div>`;
            return;
        }
        ...
        elements.aiSearchMessages.innerHTML = `<div class="ai-search-message ai-search-message-client"><small>CLIENTE</small><p>${escapeHtml(query)}</p></div><div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p>${cards.length ? `Hay ${cards.length} coincidencia${cards.length === 1 ? '' : 's'} en el catálogo.` : 'No hay coincidencias en el catálogo.'}</p></div>`;
    ...
    function openAiChatDialog() {
        ...
        const aiView = document.getElementById('view-ai-search');
        if (!aiView) return;
        const placeholder = document.createComment('Asesor IA');
        aiView.before(placeholder);
        openModal('');
        elements.modalContent.appendChild(aiView);
    ...
    function showAiUnavailable() {
        ...
        elements.aiSearchMessages.innerHTML = state.aiHistory.map(item => `<div class="ai-search-message ai-search-message-${item.role === 'user' ? 'client' : 'assistant'}"><small>${item.role === 'user' ? 'CLIENTE' : 'ASESOR IA'}</small><p>${escapeHtml(item.content)}</p></div>`).join('');
    }
    ...
    async function sendAiChat() {
        ...
        elements.aiSearchInput.value = '';
        elements.aiSearchProducts.innerHTML = '';
        elements.aiSearchMessages.innerHTML = state.aiHistory.map(item => `<div class="ai-search-message ai-search-message-${item.role === 'user' ? 'client' : 'assistant'}"><small>${item.role === 'user' ? 'CLIENTE' : 'ASESOR IA'}</small><p>${escapeHtml(item.content)}</p></div>`).join('') + '<div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p class="ai-search-progress"><i aria-hidden="true"></i>Ya te digo, voy a buscarlo en el catálogo.</p></div>';
        scrollAiChatToBottom();
    ...
    async function testAiCatalogTool() {
        ...
        try {
            ...
            elements.aiSearchMessages.innerHTML = `<div class="ai-search-message ai-search-message-client"><small>CLIENTE</small><p>${escapeHtml(query)}</p></div><div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p>${count ? `La herramienta devolvió ${count} resultado${count === 1 ? '' : 's'}.` : 'La herramienta no devolvió resultados.'}</p></div>`;
            elements.aiSearchProducts.innerHTML = '';
    ...
    function renderProducts() {
        ...
        elements.productList.innerHTML = `${toolbar}${products.length ? `
            <div class="product-list-table" role="table" aria-label="Listado de productos">
                <div class="product-list-head" role="row"><span></span><span>Producto y variantes</span><span>Stock</span><span>Precio</span><span>Estado</span><span></span></div>
            ...
            elements.orderList.innerHTML = `
                ...
                ${matchingOrders.map(order => {
                    ...
                    const stateIndicator = order.archived_at
                        ...
                        : (order.status === 'cancelled'
                            ? '<span class="order-status-indicator order-status-indicator-cancelled" role="img" aria-label="Venta cancelada" title="Venta cancelada">C</span>'
                            : '');
                    const statusDisplay = stateIndicator ? '' : `<span class="status-pill status-${escapeHtml(inDeliveries ? 'copied' : order.status)}">${escapeHtml(inDeliveries ? `EN ENTREGAS · ${deliverySlot}` : (statusLabels[order.status] || order.status))}</span>`;
                    return `
    ...
    function renderStatistics(statistics, deliveries, financialsVisible) {
        ...
        const artjetVisits = [
            ['HOY', Number(statistics.artjet_visits?.daily || 0)],
            ['SEMANA', Number(statistics.artjet_visits?.weekly || 0)],
            ['MES', Number(statistics.artjet_visits?.monthly || 0)],
            ['AÑO', Number(statistics.artjet_visits?.yearly || 0)],
        ];
        const discounts = [
        ...
        elements.statisticsContent.innerHTML = `
            ...
            <div class="statistics-grid">
                ...
                <section class="statistics-card statistics-visits-card">
                    <div><span class="statistics-icon">A</span><p class="eyebrow">VISITAS ÚNICAS · WWW.ARTJET.COM.AR</p></div>
                    <p>Personas distintas que ingresaron al catálogo desde el dominio Artjet.</p>
                    <div class="statistics-visits">${artjetVisits.map(([label, count]) => `<span><small>${label}</small><strong>${count}</strong></span>`).join('')}</div>
                </section>
    ...
    document.addEventListener('click', async event => {
        ...
        if (adjustSupplierOrderQuantity && state.supplierOrder) {
            const variantId = Number(adjustSupplierOrderQuantity.dataset.supplierOrderAdjustQuantity);
            const adjustment = Number(adjustSupplierOrderQuantity.dataset.quantityAdjustment);
            const line = state.supplierOrder.lines.find(item => Number(item.variant_id) === variantId);
            if (!line || !Number.isInteger(adjustment)) return;
            const quantity = Math.min(99999, Math.max(0, Number(line.quantity || 0) + adjustment));
            if (quantity === Number(line.quantity || 0)) return;
            updateSupplierOrderLine(variantId, { quantity });
            const input = document.querySelector(`[data-supplier-order-quantity="${variantId}"]`);
