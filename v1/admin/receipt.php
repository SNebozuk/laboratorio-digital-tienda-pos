...
<div class="receipt-items"><?php foreach ($items as $item): $imagePath = trim((string) ($item['image_path'] ?? '')); $variantName = trim((string) $item['variant_name']); ?><div class="receipt-item"><?php if (str_starts_with($imagePath, '/')): ?><img class="receipt-item-image" src="<?= receiptText($imagePath) ?>" alt=""><?php else: ?><span class="receipt-item-placeholder">SIN FOTO</span><?php endif; ?><span class="receipt-quantity"><?= (int) $item['quantity'] ?></span><span class="receipt-product"><?= receiptText($item['product_name'] ?: 'Producto') ?></span><span class="receipt-variant"><?= preg_match('/^única$/iu', $variantName) === 1 ? '' : receiptText($variantName) ?></span><span class="receipt-item-price"><?= receiptMoney((int) $item['unit_price_cents']) ?></span></div><?php endforeach; ?></div>
