(() => {
    ...
    function render() {
        ...
        const cards = tokens.length ? catalog.filter(product => productMatches(product, tokens)).flatMap(product => (product.variants || []).map(variant => ({ product, variant }))).slice(0, 12) : [];
