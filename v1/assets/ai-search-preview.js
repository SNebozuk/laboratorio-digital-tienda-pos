(() => {
    ...
    function variantMatches(product, variant, tokens) {
        const values = [product.name, product.description, product.category?.name, variant.name];
        return tokens.every(token => values.some(value => words(value).some(word => word === token || word.startsWith(token))));
    }
    ...
    function render() {
        ...
        if (!query) {
            messages.innerHTML = '<div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p>¡Hola! Soy el Asesor IA de Laboratorio Digital. ¿En qué te ayudo?</p></div>';
            products.innerHTML = '';
    ...
    fetch(`${config.apiUrl}?action=catalog`, { headers: { Accept: 'application/json' }, cache: 'no-store' })
        ...
        .catch(() => { messages.innerHTML = '<div class="ai-search-message ai-search-message-assistant"><small>ASESOR IA</small><p>No pude consultar el catálogo.</p></div>'; });
